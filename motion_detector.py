...
lightsAssigned = False
while lightsAssigned is False:
...
while True:
    ...
    curr = time.time()
    if (curr-frameStart) > 10 or static_back is None:
        static_back = gray
        frameStart = curr
        continue
    ...
    for contour in cnts:
        ...
        if (curr-checkLightStart) > 600 and deskLight.is_off and doorLight.is_off and couchLight.is_off:
            deskLight.turn_on()
            doorLight.turn_on()
            couchLight.turn_on()

        checkLightStart = curr
